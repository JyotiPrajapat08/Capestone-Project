...
if(isset($_SESSION['name'])) //Post Data received from product list page.
{
	...
	$ItemName 		= $_SESSION['name']; //Item Name
	$ItemPrice 		= $_SESSION["price"]; //Item Price
	$ItemNumber 	= "1"; //Item Number
	$ItemDesc 		= "CDN Website"; //Item Number
	...
	$plan=$_POST['prog'];
	$check=mysqli_query($con,"select * from payment where email='$memail' and plan='$plan'");
 if(mysqli_num_rows($check)>0)
 {
	 $_SESSION['error']="This program has already purchased by you.";
...
if(isset($_GET["token"]) && isset($_GET["PayerID"]))
{
	...
	if("SUCCESS" == strtoupper($httpParsedResponseAr["ACK"]) || "SUCCESSWITHWARNING" == strtoupper($httpParsedResponseAr["ACK"])) 
	{
			...
			$tid=urldecode($httpParsedResponseAr["PAYMENTINFO_0_TRANSACTIONID"]);
				...
				*/
				...
				{
					//echo '<div style="color:green">Payment Received! Your will shortly receive an email containing your username and password for login.</div>';
				}
